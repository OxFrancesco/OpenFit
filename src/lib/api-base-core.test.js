...
describe('production API base URL selection', () => {
  test('rejects IPv6 loopback in production', () => {
    expect(resolveExplicitApiBaseUrl('http://[::1]:8081', 'web', false)).toBeNull();
    expect(resolveExplicitApiBaseUrl('http://[::1]:8081', 'ios', false)).toBeNull();
  });
