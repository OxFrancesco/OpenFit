...
export default function PrivacyPolicyScreen() {
  return <LegalPage title="OpenFit Privacy Policy" updated="September 14, 2026">
    ...
    <LegalSection title="Markdown export">
      <LegalParagraph>Settings can create a Markdown file containing 30 days of available device health, all gym logs saved on this device, daily goals and a suggested AI prompt. The file is created locally. OpenFit does not send it to an AI provider. You decide where to save or share it, and the service you choose handles that copy under its own policy.</LegalParagraph>
      <LegalParagraph>The built-in coach has been removed. Previously saved conversations remain subject to the existing 90-day retention limit. Contact {OPENFIT_CONTACT_EMAIL} to request earlier deletion. OpenFit is a wellness utility and does not provide medical advice, diagnosis or treatment.</LegalParagraph>
    </LegalSection>
    ...
    <LegalSection title="Optional fitness connections">
      ...
      <LegalParagraph>Provider credentials and connection metadata are excluded from Markdown exports. OpenFit does not combine Strava data with device health, Garmin or manual workouts, and does not use Strava API data to train, evaluate, ground or operate AI systems.</LegalParagraph>
    ...
    <LegalSection title="Sharing and deletion">
      <LegalParagraph>We do not sell personal or health data, share health data with advertisers, or use it to train general-purpose AI models. Clerk handles authentication; Cloudflare handles account services.</LegalParagraph>
      <LegalParagraph>To request account and server-data deletion, contact {OPENFIT_CONTACT_EMAIL} using the account email. Disconnect fitness providers in the app when possible. Signing out ends the local session but does not delete the Clerk account or revoke Google sign-in consent.</LegalParagraph>
      <LegalParagraph>On upgrade, OpenFit removes locally stored legacy Google credentials. The server no longer uses Google Health credentials and removes retired credentials when an existing account agent is opened. Earlier coach messages retain the 90-day retention limit and can be deleted through a support request.</LegalParagraph>
    </LegalSection>
